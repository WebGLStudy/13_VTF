    ...
    window.addEventListener('load', function(){
        ...
        const fsSourceWater = [
            ...
            'void main(void) {',
                ...
                // 弾を打った場所で高さを上げる
                'float bullet_size = 1.0 / 0.1;',// 張り込むテクスチャのテクスチャ空間での広がりの逆数
                'vec2 bullet_center = vec2(0.5, 0.5);',// テクスチャの中心をvUvにするためのずらし
                'float bullet = 3.0 * texture(sampBullet, vUv * bullet_size - bullet_pos * bullet_size + bullet_center).x;',
        ...
        const vsSourceScene = [
            ...
            'void main(void) {',
                ...
                'vPosition = (mwMatrix * vec4(position.xyz, 1.0)).xyz;',// いない場合での頂点位置をフラグメントシェーダに送る
            '}'
        ].join('\n');
        ...
        const fsSourceScene = [
            ...
            'void main(void) {',
                'vec3 view_dir = normalize(camera_pos - vPosition);',// 静かな際の水面の位置の先のテクスチャをサンプリング
                'vec3 tarans = texture(sampCube, -view_dir * vec3(-1,1,1)).rgb;',// 視線の先の環境光源を読み込む
                
                'outColor = vec4(tarans, 1.0);',
            '}'
        ].join('\n');
